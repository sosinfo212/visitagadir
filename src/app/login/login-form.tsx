...
import { Building2, Mail, Lock, ArrowRight, Loader2, CheckCircle2 } from 'lucide-react'
...
export default function LoginForm() {
  ...
  useEffect(() => {
    if (verified) {
      setInfo('Your email has been verified. You can sign in now.')
      setNeedsVerification(false)
    } else if (verifyStatus === 'expired') {
      setError('Your verification link has expired. Resend a new one below.')
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground text-sm mt-1">Sign in to list your business</p>
        </div>
        ...
        <div className="bg-white border rounded-2xl shadow-sm p-6 space-y-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {info && (
              <div className="bg-green-50 border border-green-200 text-green-700 text-sm rounded-lg px-3 py-2 flex items-start gap-2">
          ...
          <p className="text-center text-sm text-muted-foreground">
            ...
            <Link href={`/signup?callbackUrl=${encodeURIComponent(callbackUrl)}`} className="text-orange-600 font-medium hover:underline">
              Sign up
            </Link>
          </p>
        </div>
